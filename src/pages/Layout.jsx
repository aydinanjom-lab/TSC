  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-black text-white">
        ...
        {/* Mobile menu button */}
        <div className="lg:hidden fixed top-4 left-4 z-50 mobile-sticky">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setIsMenuOpen(!isMenuOpen)}
            className="text-white min-h-[44px] min-w-[44px] touch-manipulation"
          >
        ...
        {/* Sidebar */}
        <div className={`fixed inset-y-0 left-0 z-40 w-64 bg-zinc-950/95 backdrop-blur-xl border-r border-zinc-800 transform transition-transform duration-200 ease-in-out lg:translate-x-0 mobile-scroll ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex flex-col h-full pt-[env(safe-area-inset-top)]">
            ...
            {/* Navigation */}
            <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto mobile-scroll">
              {navigation.map((item) => {
                ...
                return (
                  <Link
                    key={item.name}
                    to={item.href}
                    className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors min-h-[44px] touch-manipulation ${
                      isActive
            ...
            {/* User menu */}
            <div className="p-4 border-t border-zinc-800 pb-[env(safe-area-inset-bottom)]">
              {user ? (
                <div className="space-y-4">
                  ...
                  <Button
                    variant="ghost"
                    className="w-full justify-start text-zinc-300 hover:text-white hover:bg-zinc-800 min-h-[44px] touch-manipulation"
                ...
                <div className="space-y-2">
                  <Button className="w-full chrome-button min-h-[44px] touch-manipulation">
                    Sign In
                  </Button>
